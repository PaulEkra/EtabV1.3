...
def edit_prof():
    while True:
        id = input("Entrez l'id du professeur à modifier: ")
        if id.isdigit():
            break
...
def edit_choice(prof):
    try:
            ...
            if choice == "1":
                ...
                prof['personne']['nom'] = nom
                Professeur.modifier(prof)
            elif choice == "2":
                prenom=input("Entrez le prenom: ")
                prof['personne']['prenom'] = prenom
                Professeur.modifier(prof)
            elif choice == "3":
                while True:
                    try:
                        ...
                        dateNaissance = datetime.strptime(dateNaissance_str, "%d-%m-%Y").date()
                        if dateNaissance:
                            prof['personne']['date_naissance'] = dateNaissance
                            Professeur.modifier(prof)
                    except ValueError:
                        print("Erreur : Format de date invalide. Veuillez entrer la date au format JJ-MM-AAAA.")
            elif choice == "4":
                ville=input("Entrez la ville: ")
                prof['personne']['ville'] = ville
                Professeur.modifier(prof)
            elif choice == "5":
                if prof['professeur']['vacant'] == True:
                    prof['professeur']['vacant'] = False
                    Professeur.modifier(prof)
                else: 
                    prof['professeur']['vacant'] = True
                    Professeur.modifier(prof)
            elif choice == "6":
                while True:
                    tel=input("Entrez le nuémro: ")
                    if tel.isdigit():
                        prof['personne']['telephone'] = tel
                        Professeur.modifier(prof)
                        break
                    else:
                        print("le numéro doit être numérique")
                ...
                prof['professeur']['matiere_enseigne']=matiere
                Professeur.modifier(prof)
            elif choice == "8":
                prochain_cours=input("Entrez le prochain cours: ")
                prof['professeur']['prochain_cours'] = prochain_cours
                Professeur.modifier(prof)
            elif choice == "9":
                sujet_prochaine_reunion=input("Entrez le sujet de la prochaine reunion: ")
                prof['professeur']['sujet_prochaine_reunion'] = sujet_prochaine_reunion
                Professeur.modifier(prof)
            elif choice == "10":
                menu_professeur()
            elif choice == "0":
